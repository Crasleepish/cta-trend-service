...
class JobRunner:
    ...
    def _signal_flow(self, ctx: RunContext, req: RunRequest) -> RunResult:
        ...
        summary = self.signal_service.compute_and_persist_signals(
            run_id=ctx.run_id,
            strategy_id=ctx.strategy_id,
            version=ctx.version,
            snapshot_id=ctx.snapshot_id,
    ...
    def _full_flow(self, ctx: RunContext, req: RunRequest) -> RunResult:
        ...
        signal_summary = self.signal_service.compute_and_persist_signals(
            run_id=ctx.run_id,
            strategy_id=ctx.strategy_id,
            version=ctx.version,
            snapshot_id=ctx.snapshot_id,
        ...
        signal_upsert = (
            signal_summary.rows_upserted if hasattr(signal_summary, "rows_upserted") else 0
        )
