...
对每个基金 $i\in F$，取因子暴露向量（建议使用系统长期因子集合）：

$$
\beta_i=
(\beta_{i,1},\dots,\beta_{i,d})\in\mathbb{R}^d.
$$

同时有 beta 估计不确定性信息（来自 Kalman/回归输出），例如每维方差 $Var(\beta_{i,j})$。

定义一个简化的不稳定度指标（示例）：

$$
U_i=\frac{1}{d}\sum_{j=1}^d \sqrt{Var(\beta_{i,j})}.
$$
...
$$
U_i \le U_{\max}
\quad (\text{或按维度阈值 } \sqrt{Var(\beta_{i,j})}\le u_{\max,j}).
$$

过滤后得到稳定集合 $F'\subseteq F$。被过滤资产可保留在“观察/候补池”，但不参与聚类中心形成。

### 3.2 Beta 鲁棒标准化 + 单位化

在 $F'$ 上对每一维做鲁棒尺度估计（MAD 或 IQR）。以 MAD 为例：

$$
a_j=\mathrm{MAD}(\{\beta_{i,j}\}_{i\in F'})=\mathrm{median}_i\bigl(|\beta_{i,j}-\mathrm{median}(\beta_{\cdot,j})|\bigr)+\varepsilon.
$$

构造对角矩阵 $D=\mathrm{diag}(a_1,\dots,a_d)$，缩放：

$$
\tilde\beta_i = D^{-1}\beta_i.
$$
