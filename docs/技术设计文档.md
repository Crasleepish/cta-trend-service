...
> 白皮书请查看：docs/CTA_System_Whitepaper.md
---
...
- 风险 bucket 集合 **可配置**（用于归一化与风险袖套分配），防御 bucket **固定为 RATE/CASH** 且与防御逻辑强绑定
...
portfolio:
  # 风险 bucket 可配置；防御 bucket 固定 RATE/CASH
...
## 12. Bucket Tradable Assets 推荐工具

该工具用于为某个 bucket（例如 GROWTH）自动检索并推荐一组可交易资产（基金）列表。详细设计见：docs/附录1-BucketTradableAssets推荐工具设计方案.md
